...
import { disconnectNuvemshop, syncNuvemshopOrders } from "@/server/integrations/nuvemshop";
...
/** Sincroniza os pedidos pagos recentes do Nuvemshop (fallback do webhook). */
export async function syncOrders(formData: FormData): Promise<void> {
  await requireUser();
  const tenantId = String(formData.get("tenantId") ?? "");
  if (!tenantId) return;
  const supabase = await createSupabaseServerClient();
  ...
  await syncNuvemshopOrders(tenantId);
  revalidatePath(`/${tenantId}/conversions`);
  revalidatePath(`/${tenantId}`);
}
